...
public abstract class BaseMultiItemAdapter<T extends MultiItemEntity, V extends BaseViewHolder> extends BaseQuickAdapter<T, V> {
    ...
    protected DelegateProvider mProvider;
    ...
    protected void initialize() {
        if (mProvider == null)
            mProvider = new DelegateProvider();
        registerDelegate();
        registerItem();
    }

    private void registerItem() {
        SparseArray delegates = mProvider.getItemDelegates();
        for (int i = 0; i < delegates.size(); i++) {
            int key = delegates.keyAt(i);
            BaseItemDelegate delegate = (BaseItemDelegate) delegates.get(key);
            getMultiTypeDelegate().registerItemType(key, delegate.getLayoutId());
        }
        setMultiTypeDelegate(new MultiTypeDelegate<T>() {
            ...
            protected int getItemType(T t) {
                if (!useItemDelegate())
                    return mProvider.getDefItemType();
                return mProvider.getItemViewType(t);
            }
        });
    }

    private boolean useItemDelegate() {
        return mProvider.getDelegateCount() > 0;
    }
    ...
    protected V onCreateDefViewHolder(ViewGroup parent, int viewType) {
        ...
        View itemView = delegate.getItemView();
    ...
    private void bindClick(final V helper, final T item, final int position, final BaseItemDelegate delegate) {
        if (!isEnabled(delegate.getViewType()))
            return;
